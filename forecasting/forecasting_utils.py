# COPIED (with slight modification) FROM https://github.com/interpolants/forecasting/blob/main/navier-stokes/utils.py
...
class LatentDriftMLP(nn.Module):
    def __init__(self, latent_dim, hidden_dim=512, num_layers=4):
        super().__init__()
        # time embedding
        ...
        # main MLP: input = [x_t , cond , time_emb]
        in_dim = latent_dim + latent_dim + hidden_dim      # x_t + cond + t_emb
        layers = []
        for _ in range(num_layers - 1):
            layers += [nn.Linear(in_dim, hidden_dim), nn.SiLU()]
            in_dim = hidden_dim
        layers.append(nn.Linear(hidden_dim, latent_dim))   # output drift
        self.net = nn.Sequential(*layers)

    def forward(self, x_t, t, cond=None):
        """
        x_t  : [B, latent_dim]  interpolant sample at time t
        t    : [B]              scalar times
        cond : [B, latent_dim]  conditioning vector (here z0)
        """
        if cond is None:
            raise ValueError("cond (z0) must be provided for conditional drift (used for forecasting)")
        #print("TIME: ", t.shape)
        t_emb = self.time_mlp(t[:, None])                  # [B, hidden_dim]
        #print("XT: ", x_t.shape)
        #print("COND: ", cond.shape)
        #print("T_EMB: ", t_emb.shape)
        h = torch.cat([x_t, cond, t_emb], dim=-1)      # concat
        return self.net(h)                                 # [B, latent_dim]
...
class DriftModel(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        c = config
        self._arch = LatentDriftMLP(c.latent_dim, c.hidden_dim, c.num_layers)
...
def build_forecasting_dataset(latents, lag, stride, random_time = False):
    conds = []
    targets = []
    latents = latents[:int(0.8*len(latents)), :]

    if random_time:
        T = len(latents)
        starts = torch.randint(0, T-lag, (T//2,))
        conds = latents[starts]
        targets = latents[starts + lag]
        
    else: 
        for t in range(0, len(latents) - lag, stride):
            conds.append(latents[t])
            targets.append(latents[t + lag])
        conds, targets = torch.stack(conds), torch.stack(targets)

    return TensorDataset(conds, targets)

def build_forecasting_testset(latents, lag, stride, random_time=False):
    conds = []
    targets = []
    print("LATENTS FIRST INDEX:", int(0.8*len(latents)))
    latents = latents[int(0.8*len(latents)):, :]
    for t in range(0, len(latents)-lag, stride):
        conds.append(latents[t])
        targets.append(latents[t+lag])
...
def get_forecasting_dataloader(config, shuffle=False):
    ...
    latents = torch.load(os.path.join(config.data_path, config.latent_filename), weights_only=True)
    dataset = build_forecasting_dataset(latents, config.time_lag, config.stride, config.random_time_sampling)

    return DataLoader(dataset, batch_size=config.batch_size, num_workers= config.num_workers, shuffle=shuffle)

def get_test_dataloader(config):
    latents = torch.load(os.path.join(config.data_path, config.latent_filename))
    dataset = build_forecasting_testset(latents, config.time_lag, config.stride, config.random_time_sampling)
    return DataLoader(dataset, batch_size=config.sampling_batch_size, num_workers=config.num_workers, shuffle=False)
