...
def get_args():    
    ...
    parser.add_argument('--load', type=int, default=None,
                        help='Completed epoch to load model state dict in. The path must be provided in the --model_dir argument.')
    ...
    parser.add_argument('--norm', type=bool, default=False,
                        help='Normalize input coordinates. Default=False')
...
if __name__ == "__main__":
    ...
    if args.data == 'pentapeptide':
        ...
        raise ValueError("We currently do not support datasets other than pentapeptide and fs peptide or the two DESRES trajectories.")
    ...
    if args.model_dir:
        ...
        model_dir = f'/scratch/users/lreeder/gcae_experiments/TCN_{model_type.lower()}_training/{args.data}/{normalize}norm_{smooth}smoothing_{epochs}ep_{lr}lr_{drop_rate}dr_{batch_size}batch_{n_layers}layers_{top_k}edge_{latent_dim}latentdim_{lambda_reg}reg_{lambda_temp}temp_TCN2layers_TCN3kernel/'
    ...
    train_seq_dataset = dataset.SequenceDataset([len(traj0)] * len(train_files), sequence_length=256, stride=128)

    val_frame_dataset = dataset.LigandDataset(val_structures, top_k=top_k)

    val_seq_dataset = dataset.SequenceDataset([len(traj0)] * len(val_files), sequence_length=256, stride=128)

    test_frame_dataset = dataset.LigandDataset(test_structures, top_k=top_k)

    test_seq_dataset = dataset.SequenceDataset([len(traj0)] * len(test_files), sequence_length=256, stride=256)
    ...
    if model_type == 'GCAE':  
        ...
        tcn = TCNModel(input_size=latent_dim, channel_size=[64,64], input_length=256, kernel_size=3).to(device)
    ...
    if args.load is not None:
        ...
        tcn_dict = torch.load(os.path.join(model_dir, f"tcn_epoch-{e}.pt"), map_location=device, weights_only=True)
        encoder.load_state_dict(encoder_dict)
        decoder.load_state_dict(decoder_dict)
        tcn.load_state_dict(tcn_dict)
        epoch_start = e + 1 
    ...
    for epoch in tqdm(range(epoch_start, epochs), desc="Training Progress", leave=True):
        ...
        if (epoch + 1) % val_int == 0:
            ...
            tcn_path = os.path.join(model_dir, f"tcn_epoch-{epoch}.pt")
